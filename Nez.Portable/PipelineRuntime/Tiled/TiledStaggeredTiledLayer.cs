...
namespace Nez.Tiled
{
	public class TiledStaggeredTiledLayer : TiledTileLayer
	{
		public TiledStaggeredTiledLayer( TiledMap map, string name, int width, int height, TiledTile[] tiles ) : base(map, name, width, height, tiles)
		{ }


		public TiledStaggeredTiledLayer( TiledMap map, string name, int width, int height ) : this( map, name, width, height, new TiledTile[width * height] )
		{ }
        ...
        public override void draw( Batcher batcher, Vector2 position, Vector2 scale, float layerDepth, RectangleF cameraClipBounds)
        {
			...
			for( var y = min.Y; y <= max.Y; y++ )
			{
				for( var x = min.X; x <= max.X; x++ )
				{
					...
					var tileworldpos = tiledMap.staggeredTileToWorldPosition( new Point( x, y ) );
		...
		public void drawTile( Batcher batcher, Vector2 position, float layerDepth, int x, int y, float scale )
		{
			...
			var t = tiledMap.staggeredTileToWorldPosition( new Point( x, y ) );
			var tileRegion = tile.textureRegion;
			...
			// if we had no rotations (diagonal flipping) shift our y-coordinate to account for any non-tileSized tiles to account for
		...
		{
			var tilePos = tiledMap.staggeredTileToWorldPosition( new Point( (int) pos.X, (int) pos.Y) );
		...
		public new List<TiledTile> getTilesIntersectingBounds( Rectangle bounds )
		{
			...
			var topLeft = tiledMap.staggeredTileToWorldPosition( new Point ( bounds.X, bounds.Y ) );
			var bottomLeft = tiledMap.staggeredTileToWorldPosition( new Point ( bounds.X, bounds.Bottom ) );
			var topRight = tiledMap.staggeredTileToWorldPosition( new Point ( bounds.Right, bounds.Y ) );
			var bottomRight = tiledMap.staggeredTileToWorldPosition( bounds.Right, bounds.Bottom );
			...
			for( var x = topLeft.X; x <= bottomRight.X; x++ )
			{
				for( var y = topRight.Y; y <= bottomLeft.Y; y++ )
				{
					...
					if( tile != null && bounds.Contains( tiledMap.staggeredTileToWorldPosition( (int) x, (int) y ) ) )
						tilelist.Add( tile );
				}
			}
		...
		public new TiledTile linecast( Vector2 start, Vector2 end )
		{
			...
			// worldToTilePosition clamps to the tilemaps bounds so no need to worry about overflow
			var startCell = tiledMap.staggeredWorldToTilePosition( start );
			var endCell = tiledMap.staggeredWorldToTilePosition( end );
			...
			// If negative, cell is before this one in which case don't add to boundary
